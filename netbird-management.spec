...
%description
Optional management server component for netbird. Please note that this does not
...
go build -o netbird-mgmt -buildmode=pie -ldflags "$LDFLAGS" ./management

%install
install -Dm755 -t %{buildroot}%{_bindir} netbird-mgmt

# Generate completions
for sh in bash zsh fish; do
  ./netbird-mgmt completion $sh > netbird-mgmt.${sh}
done
install -Dm644 netbird-mgmt.bash %{buildroot}%{_datadir}/bash-completion/completions/netbird-mgmt
install -Dm644 netbird-mgmt.zsh  %{buildroot}%{_datadir}/zsh/site-functions/_netbird-mgmt
install -Dm644 netbird-mgmt.fish %{buildroot}%{_datadir}/fish/vendor_completions.d/netbird-mgmt.fish
...
%{_datadir}/bash-completion/completions/netbird-mgmt

%files fish-completion
%{_datadir}/fish/vendor_completions.d/netbird-mgmt.fish
